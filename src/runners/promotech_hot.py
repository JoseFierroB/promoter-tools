#!/usr/bin/env python3
"""PromoTech RF-HOT runner — PG mode (sliding window + max aggregation)."""
import argparse
import os
import shutil
import subprocess
...
def main():
    p = argparse.ArgumentParser(description="PromoTech RF-HOT PG prediction")
    ...
    for label, fasta in [("pos", args.pos), ("neg", args.neg)]:
        ...
        subprocess.run(
            [pt_python, "promotech.py", "-pg", "-m", "RF-HOT", "-f", fasta_abs, "-o", str(od)],
            cwd=str(PROMOTECH_DIR), env=run_env,
            capture_output=True, text=True, timeout=300, check=True)
        subprocess.run(
            [pt_python, "promotech.py", "-g", "-m", "RF-HOT",
    ...
    print(f"PromoTech RF-HOT: {pos_count + neg_count} seqs in {elapsed:.1f}s")
