...
TSS = ROOT / 'data/benchmark/positives_81bp_metadata.tsv'
...
def wmeme(fh,name,pwm):
    w=len(pwm['A']); fh.write(f'MOTIF {name}
...
def extract():
    ...
    with open(TSS,newline='') as f:
        for r in csv.DictReader(f,delimiter='	'):
            p=int(r['TSS_Position_0based']);st=r['Strand'];sid=r['Sequence_ID'];ch=r['Chromosome']
            s,e=(max(0,p-100),p) if st=='+' else (p,p+100)
            ls.append(f'{ch}	{s}	{e}	{sid}	0	{st}
')
    b.write_text(''.join(ls))
    subprocess.run([str(BEDTOOLS),'getfasta','-fi',str(GENOME),'-bed',str(b),'-fo',str(fa),'-name+'],check=True,capture_output=True)
    return fa

def fimo(meme,fa,thresh):
    o=Path(f'/tmp/fimo_{thresh}'); o.mkdir(exist_ok=True)
    subprocess.run([str(FIMO),'--thresh',str(thresh),'--text','--norc',str(meme),str(fa),'-o',str(o)],check=True,capture_output=True)
    return o/'fimo.tsv'
...
def parse(tsv):
    ...
    with open(tsv) as f:
        for r in csv.DictReader(f,delimiter='	'):
            ...
            h[s].append({'m':r.get('motif_id',r.get('motif_alt_id','')).strip().lower(),'p':float(r.get('p-value',1)),'s':int(r.get('start',0)),'e':int(r.get('stop',0))})
    return h
...
build_meme('/tmp/rpod.meme',[('RpoD_minus35',M35),('RpoD_minus10',M10),('ComX_box',CMX),('ComE_box',CME),('Extended_minus10',EXT)])
...
print(f'
SigA recovered: {c_siga}/{sum(1 for r in res if r["original_sigma"]=="SigA")} ({c_siga/sum(1 for r in res if r["original_sigma"]=="SigA")*100:.1f}%)')
print(f'SigX recovered: {c_sigx}/{sum(1 for r in res if r["original_sigma"]=="SigX")}')

cats=Counter(r['predicted_sigma'] for r in res)
print(f'
All TSS ({len(res)}):')
for c in ['RpoD_complete','RpoD_partial','ComX','ComE','unassigned']: print(f'  {c:<20} {cats.get(c,0):>4}')
nr=[r for r in res if r['original_sigma']=='None']
nc=Counter(r['predicted_sigma'] for r in nr)
print(f'
None TSS ({len(nr)}):')
for c in ['RpoD_complete','RpoD_partial','ComX','ComE','unassigned']: print(f'  {c:<20} {nc.get(c,0):>4} ({nc.get(c,0)/len(nr)*100:.0f}%)')

# Export
with open(OUT/'d39v_sigma_assigned.tsv','w',newline='') as f:
    w=csv.DictWriter(f,fieldnames=['tss_id','original_sigma','predicted_sigma'],delimiter='	')
