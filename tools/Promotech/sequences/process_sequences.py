import sys, os, numpy as np, pandas as pd, time, joblib, pickle
# tensorflow imported lazily below (only needed for GRU/LSTM models)
...
from utils import fastaToHotEncodingSequences, charToBinary, print_fn, fastaToTetranucleotides, tetranucleotideToStringSentences, fastaToTetranucletideTable, dataConverter, predict_proba
...
def predictSequencesFromString(sequences_str, print_fn=print_fn, out_dir="RF-HOT", threshold=0.5, model_type="RF-HOT" , log_file=None, start_time=time.time()):
  ...
  model = None
  # Model paths: use research NFS for large model files, fallback to local models/
