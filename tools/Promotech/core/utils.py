...
# Replaced skbio.Sequence.kmer_frequencies with pure Python
from collections import Counter
from itertools import product

def predict_proba(model, X):
    """Predict probabilities, handling both sklearn and Keras models."""
    pred = getattr(model, 'predict_proba', model.predict)
    return pred(X)

def kmer_frequencies(seq, k=4, overlap=True, relative=True):
    """Compute k-mer frequencies. Replaces skbio.Sequence.kmer_frequencies."""
    counts = Counter()
...
def promoterToTetraFreq(seq_arr):
  ...
  for i, s in enumerate(seq_arr):
    ...
    if(len(s) < 40 ):
      nucleotide_counter = nucleotide_counter + 1
      continue
    seq_dict = kmer_frequencies( s, k=4, overlap=True, relative=True)
    tetra_n[i] = seq_dict
    bar.update(i)
...
def fastaToTetraNucletideDic(seqArray, label):
  ...
  for i, s in enumerate(seqArray):
    ...
    if(len(s[0]) < 40 ):
      ...
      sequence_arr[i] = dict()
      continue
    seq = kmer_frequencies(
        s[0], k=4, overlap=True, relative=True)
    sequence_arr[i] = seq
    bar.update(i)
...
def fastaToTetranucletideTable(seqs):
  ...
  for i, s in enumerate(seqs):
    ...
    if(len(s) < 40 ):
      ...
      sequence_arr[i] = dict()
      continue
    seq = kmer_frequencies(s, k=4, overlap=True, relative=True)
    sequence_arr[i] = seq
    bar.update(i)
